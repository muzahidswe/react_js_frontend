...
export async function getPlaces(selectedFi){
  try {
...
//   }
// }
export async function getDashboardData(dpIds, dateRange, requestType){
    const token = localStorage.getItem('token');
    const config = {
        headers: { 
            Authorization: `Bearer ${token}`,
    ...
    dpIds.length === 0 ? dpIds=[] : dpIds=dpIds; 
    const data = await axios.post(DASHBOARD_URL, {dpIds, dateRange, requestType}, config);
    return data.data;
}
